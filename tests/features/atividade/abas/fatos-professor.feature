Feature: Aba Fatos - Tela de atividade
	Como Professor
	Eu quero criar ou excluir ou visualizar fatos de atividades
...
Scenario: Acesso a aba fatos com fatos criados
		...
		And os fatos inseridos exibem o ícone de exclusão
		And o botão "Adicionar Fato" está habilitado
...
Scenario: Exclusão de fatos em atividade
		Given eu acesso a aba fatos de uma atividade em estado posterior ou igual a fatos
		When eu clico no ícone de exclusão do fato
		Then a tela exibe a mensagem de confirmação "Texto ainda não definido" com os botões "Confirmar" e "Cancelar"
		And quando eu clico em "Confirmar" a tela apaga o fato
